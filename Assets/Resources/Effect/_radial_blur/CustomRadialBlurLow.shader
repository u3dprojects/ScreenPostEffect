Shader "Custom/RadialBlurLow"
{
	Properties   
    {  
        _MainTex ("Base (RGB)", 2D) = "white" {}  
        _BlurFactor("Amount",Range(0.0,1.0)) = 0.1
        _BlurCenter("Center Ponit",Vector) = (0.5,0.5,0.0,0.0)
    }  
        ...
        {  
		    ...
		    {  
		        ...
		        for (int j = 0; j < SAMPLE_COUNT; ++j)  
		        {
		            ...
		            scale = 1.0 + _BlurFactor * j / (SAMPLE_COUNT - 1);
		            outColor += tex2D(_MainTex, dir * scale + _BlurCenter);  
		        }  
		        //取平均值  
