...
public class DepthOfFiled : ScreenPostEffectBase
{
	...
	void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		if (m_material)
		{
			...
			Graphics.Blit(temp2, temp1, m_material, 0);

			//景深操作，景深需要两的模糊效果图我们通过_BlurTex变量传入shader
			m_material.SetTexture("_BlurTex", temp1);
			//设置shader的参数，主要是焦点和远近模糊的权重，权重可以控制插值时使用模糊图片的权重
			m_material.SetFloat("_focalDistance", FocalDistance01(focalDistance));
			m_material.SetFloat("_nearBlurScale", nearBlurScale);
			m_material.SetFloat("_farBlurScale", farBlurScale);
